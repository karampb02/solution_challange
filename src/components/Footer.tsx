    ...
    <footer className="relative pt-20 pb-10 border-t border-white/[0.04]">
      <div className="max-w-7xl mx-auto px-6">
        ...
        <motion.div
          ...
          viewport={{ once: true }}
          className="glass-card gradient-border p-10 md:p-16 text-center mb-20 relative overflow-hidden"
        >
        ...
        <div className="grid md:grid-cols-4 gap-10 mb-16">
          ...
          <div>
            ...
            <p className="text-xs text-[#55556a] leading-relaxed max-w-xs">
              AI-driven talent intelligence platform that uses Gemini and
              semantic search to match technical talent with zero manual effort.
            </p>
          </div>
          ...
          {footerLinks.map((group) => (
            <div key={group.title}>
              <h4 className="text-sm font-semibold mb-4 text-[#8888a0]">
                {group.title}
              </h4>
              <ul className="space-y-2.5">
                {group.links.map((link) => (
                  <li key={link.name}>
                    <a
                      href={link.href}
                      className="text-xs text-[#55556a] hover:text-[#8888a0] transition-colors duration-200"
                    >
                      {link.name}
                    </a>
                  </li>
